@php
    $agentUser = auth()->user();
    $agentNotifications = $agentUser->notifications()->latest()->limit(15)->get();
    $agentUnreadCount = $agentUser->unreadNotifications()->count();
@endphp
...
<div style="position:relative;">
    <button id="notif-toggle" style="width:40px;height:40px;border-radius:11px;border:1px solid var(--border);background:var(--surface);color:var(--text);cursor:pointer;display:grid;place-items:center;position:relative;transition:.15s;" onmouseover="this.style.borderColor='var(--accent)';this.style.color='var(--accent)'" onmouseout="this.style.borderColor='var(--border)';this.style.color='var(--text)'">
        ...
        <span id="notif-badge" style="display:{{ $agentUnreadCount > 0 ? 'grid' : 'none' }};position:absolute;top:5px;right:6px;min-width:17px;height:17px;border-radius:20px;background:#f43f5e;border:2px solid var(--surface);color:#fff;font-size:9px;font-weight:800;place-items:center;padding:0 3px;">{{ $agentUnreadCount > 9 ? '9+' : $agentUnreadCount }}</span>
    </button>
            ...
            @forelse($agentNotifications as $notification)
                ...
                <a href="{{ route('agent.notifications.open', $notification) }}" style="display:flex;gap:12px;padding:13px 16px;border-bottom:1px solid var(--border);text-decoration:none;transition:.12s;{{ $isUnread ? 'background:var(--primsoft);' : '' }}" onmouseover="this.style.background='var(--surface2)'" onmouseout="this.style.background='{{ $isUnread ? 'var(--primsoft)' : 'transparent' }}'">
                    ...
                    <div style="flex:1;min-width:0;">
                        ...
                        @if($notifRef)
                            <div style="font-size:11px;color:var(--accent);font-weight:700;margin-top:3px;">{{ $notifRef }}</div>
                        @endif
