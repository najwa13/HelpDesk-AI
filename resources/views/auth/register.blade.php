...
<head>
    ...
    <title>Inscription — HelpDesk AI</title>
    ...
    <div style="width:100%;min-height:100vh;display:grid;grid-template-columns:1.05fr .95fr;">
        ...
        <div style="position:relative;overflow:hidden;background:linear-gradient(150deg,#7c3aed,#5b21b6);display:flex;flex-direction:column;justify-content:space-between;padding:56px 52px;color:#fff;">
            ...
            <div style="position:relative;max-width:440px;">
                ...
                <h1 style="font-size:38px;line-height:1.12;font-weight:800;margin:0 0 16px;letter-spacing:-1px;text-wrap:balance;">Rejoignez une plateforme de support nouvelle génération.</h1>
                <p style="font-size:16px;line-height:1.6;opacity:.9;margin:0;">Créez votre compte et accédez à un espace de support intelligent propulsé par l'intelligence artificielle.</p>
            </div>
        ...
        <div style="display:flex;align-items:center;justify-content:center;padding:40px;background:var(--bg);">
            <div style="width:100%;max-width:380px;animation:fadeUp .5s both;">
                <h2 style="font-size:26px;font-weight:800;margin:0 0 6px;color:var(--text);letter-spacing:-.5px;">Inscription</h2>
                <p style="color:var(--text2);margin:0 0 28px;font-size:14px;">Créez votre compte pour commencer.</p>
                ...
                <form method="POST" action="{{ route('register') }}">
                    ...
                    <label style="display:block;font-size:13px;font-weight:600;color:var(--text);margin-bottom:7px;">Nom complet</label>
                    <input
                        type="text"
                        name="name"
                        value="{{ old('name') }}"
                        ...
                        style="width:100%;padding:13px 15px;border-radius:12px;border:1px solid {{ $errors->has('name') ? '#dc2626' : 'var(--border)' }};background:var(--surface);color:var(--text);font-size:14px;font-family:inherit;margin-bottom:18px;outline:none;transition:.15s;"
                    >
                    ...
                    <input
                        ...
                        placeholder="••••••••••"
                        style="width:100%;padding:13px 15px;border-radius:12px;border:1px solid {{ $errors->has('password') ? '#dc2626' : 'var(--border)' }};background:var(--surface);color:var(--text);font-size:14px;font-family:inherit;margin-bottom:18px;outline:none;transition:.15s;"
                    >

                    <label style="display:block;font-size:13px;font-weight:600;color:var(--text);margin-bottom:7px;">Confirmer le mot de passe</label>
                    <input
                        type="password"
                        ...
                        autocomplete="new-password"
                        placeholder="••••••••••"
                        style="width:100%;padding:13px 15px;border-radius:12px;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:14px;font-family:inherit;margin-bottom:22px;outline:none;transition:.15s;"
                    >
                ...
                <p style="text-align:center;margin-top:20px;font-size:13px;color:var(--text2);">
                    Déjà un compte ?
                    <a href="{{ route('login') }}" style="color:var(--accent);font-weight:600;text-decoration:none;">Se connecter</a>
                </p>
            </div>
        </div>
    </div>
