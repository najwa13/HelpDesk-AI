...
<div style="animation:fadeUp .45s both;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;flex-wrap:wrap;gap:12px;">
        <div>
            <div style="font-weight:800;font-size:18px;color:var(--text);">Tous les articles</div>
            <div style="font-size:13px;color:var(--muted);margin-top:2px;">{{ $articles->total() }} article(s) · publiés et brouillons</div>
        </div>
        <a href="{{ route('admin.knowledge.create') }}" style="display:inline-flex;align-items:center;gap:7px;padding:11px 18px;border:none;border-radius:11px;background:linear-gradient(135deg,var(--accent),var(--accent2));color:#fff;font-weight:700;font-size:13px;font-family:inherit;cursor:pointer;text-decoration:none;box-shadow:0 7px 16px -6px var(--accent);transition:.15s;" onmouseover="this.style.transform='translateY(-1px)'" onmouseout="this.style.transform='none'">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14M5 12h14"></path></svg>
            Nouvel article
        </a>
    </div>
        ...
        <aside style="background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:20px;box-shadow:var(--shadow);height:fit-content;position:sticky;top:100px;">
            ...
            <form method="GET" action="{{ route('admin.knowledge.index') }}">
                <div style="margin-bottom:16px;">
                    ...
                    <select name="category" style="width:100%;padding:10px 12px;border-radius:10px;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:13px;font-family:inherit;cursor:pointer;outline:none;">
                ...
                <div style="margin-bottom:16px;">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Titre, contenu…" style="width:100%;padding:10px 12px;border-radius:10px;border:1px solid var(--border);background:var(--surface2);color:var(--text);font-size:13px;font-family:inherit;outline:none;transition:.15s;" onfocus="this.style.borderColor='var(--accent)';this.style.boxShadow='0 0 0 3px var(--primsoft)'" onblur="this.style.borderColor='var(--border)';this.style.boxShadow='none'">
                </div>

                <button type="submit" style="width:100%;padding:10px;border:none;border-radius:10px;background:linear-gradient(135deg,var(--accent),var(--accent2));color:#fff;font-weight:700;font-size:13px;font-family:inherit;cursor:pointer;transition:.15s;">Appliquer</button>
            </form>
        </aside>
        ...
        <main style="flex:1;min-width:0;">
            @if($articles->isNotEmpty())
                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:16px;">
                    @foreach($articles as $article)
                        <article style="background:var(--surface);border:1px solid var(--border);border-radius:16px;padding:20px;box-shadow:var(--shadow);transition:transform .15s,box-shadow .2s;cursor:pointer;display:flex;flex-direction:column;" onclick="window.location='{{ route('admin.knowledge.show', $article) }}'" onmouseover="this.style.transform='translateY(-3px)';this.style.boxShadow='0 16px 40px -12px rgba(90,40,180,.25)'" onmouseout="this.style.transform='none';this.style.boxShadow='var(--shadow)'">
                            ...
                            <div style="display:flex;justify-content:space-between;align-items:center;padding-top:14px;border-top:1px solid var(--border);font-size:12px;color:var(--muted);margin-top:auto;">
                                <span style="display:flex;align-items:center;gap:6px;">#KB-{{ str_pad((string) $article->id, 2, '0', STR_PAD_LEFT) }}</span>
                                <a href="{{ route('admin.knowledge.edit', $article) }}" onclick="event.stopPropagation();" style="color:var(--accent);font-weight:700;text-decoration:none;">Modifier</a>
                            </div>
                        </article>
                    @endforeach
                </div>
            ...
            @else
                <div style="text-align:center;padding:64px 32px;color:var(--muted);">
                    ...
                    <div style="font-weight:700;font-size:16px;color:var(--text);margin-bottom:6px;">Aucun article</div>
                    <div style="font-size:13px;color:var(--text2);">Créez votre premier article de la base de connaissances.</div>
                </div>
            @endif
        </main>
    </div>
