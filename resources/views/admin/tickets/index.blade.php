...
@section('pageTitle', 'Tous les tickets')
...
<div style="animation:fadeUp .45s both;">
    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:18px;">
        @foreach($filterMap as $label => $val)
            ...
            @endphp
            <a href="{{ $val ? route('admin.tickets.index', array_merge(request()->query(), ['statut' => $val])) : route('admin.tickets.index') }}"
                ...
                <table style="width:100%;border-collapse:collapse;font-size:13px;">
                    <thead>
                        <tr style="text-align:left;color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.4px;">
                            ...
                            <th style="padding:12px 10px;font-weight:700;">Agent</th>
                    ...
                    <tbody>
                        @foreach($tickets as $ticket)
                            <tr style="border-top:1px solid var(--border);cursor:pointer;transition:.12s;" onmouseover="this.style.background='var(--surface2)'" onmouseout="this.style.background='transparent'" onclick="window.location='{{ route('admin.tickets.show', $ticket['id']) }}'">
                                <td style="padding:14px 10px;">
                                    <div style="font-weight:700;color:var(--text);">#{{ $ticket['id'] }} · {{ $ticket['titre'] }}</div>
                                ...
                                <td style="padding:14px 10px;color:var(--text2);font-weight:600;">{{ $ticket['agent']['name'] ?? 'Non assigné' }}</td>
